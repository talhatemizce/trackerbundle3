  ...
  const saveIsbnRule = async () => {
    ...
    try {
      if (isbnNewMax || isbnUsedMax) {
        await req(`/rules/${selected}/override`, {
      ...
      <div style={{
        ...
        <div style={{ padding: "16px 18px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "flex-start", gap: 14, flexShrink: 0 }}>
          ...
          <div
            ...
            style={{
              width: 110, height: 155, flexShrink: 0, borderRadius: 8, overflow: "hidden",
              background: C.surface2, border: `1px solid ${C.border}`,
            ...
            <>
              ...
              <AccordionSection title="📈 Fiyat Geçmişi (Keepa)" C={C} defaultOpen={true}>
                ...
                <div style={{ display: "flex", gap: 8, justifyContent: "center", flexWrap: "wrap", marginTop: 4 }}>
                  <a href={`https://keepa.com/#!search/1-${isbn13to10(isbn)}`} target="_blank" rel="noreferrer"
                    style={{
                      flex: 1, fontSize: 11, color: "white", textDecoration: "none", padding: "8px 12px",
              ...
              <AccordionSection title="📚 En Ucuz Fiyat Bul" C={C} defaultOpen={true}>
                {(() => {
                  ...
                  const SRC = {
                    bookfinder: "📚 BookFinder", abebooks: "📖 AbeBooks", thriftbooks: "♻️ ThriftBooks",
                    ...
                    <div>
                      ...
                      {!isLoading && (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginBottom: 8 }}>
                          <button onClick={() => doFetch("used")} style={{
                            padding: "9px 4px", borderRadius: 6, fontSize: 11, fontWeight: 700,
                          ...
                          <button onClick={() => doFetch("new")} style={{
                            padding: "9px 4px", borderRadius: 6, fontSize: 11, fontWeight: 700,
                          ...
                          <button onClick={() => doFetch("all")} style={{
                            gridColumn: "1/-1", padding: "6px", borderRadius: 5, fontSize: 10, fontWeight: 600,
                        ...
                        return (
                          <div>
                            ...
                            {cheapest && (() => {
                              ...
                              return (
                                <div style={{
                                  background: C.surface2, border: `1.5px solid ${chCol}66`,
                                ...
                                {allOffers.slice(0, 20).map((o, i) => {
                                  ...
                                  return (
                                    <tr key={i} style={{ borderBottom: `1px solid ${C.border}`, background: i === 0 ? C.surface2 : "" }}>
                                      ...
                                      <td style={{
                                        padding: "4px", textAlign: "right", fontWeight: 700, fontFamily: "var(--mono)",
                                        color: i === 0 ? (o._new ? (C.green || "#22c55e") : (C.accent || "#3b82f6")) : C.text
                                      }}>${o.total}</td>
                                    </tr>
                                  );
                                })}
        ...
        <div style={{ padding: "10px 14px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap" }}>
          ...
          <a href={buildEbaySearchUrl({ isbn })} target="_blank" rel="noreferrer"
            style={{
              flex: 1, minWidth: 70, padding: "8px 6px", borderRadius: 7, background: "#e53238", color: "white",
          ...
          <a href={alertEntry?.asin ? `https://www.amazon.com/dp/${alertEntry.asin}` : `https://www.amazon.com/s?k=${isbn}&i=stripbooks`}
            target="_blank" rel="noreferrer"
            style={{
              flex: 1, minWidth: 70, padding: "8px 6px", borderRadius: 7, background: "#FF9900", color: "white",
              textDecoration: "none", textAlign: "center", fontWeight: 700, fontSize: 11, fontFamily: "var(--mono)"
            }}>
            🛒 Amazon
          </a>
          ...
          <a href={`https://www.bookfinder.com/isbn/${isbn}/`} target="_blank" rel="noreferrer"
            style={{
              flex: 1, minWidth: 70, padding: "8px 6px", borderRadius: 7, background: "#6366f1", color: "white",
              textDecoration: "none", textAlign: "center", fontWeight: 700, fontSize: 11, fontFamily: "var(--mono)"
            }}>
            📚 BookFinder
          </a>
          ...
          {alertEntry?.url && (
            <a href={alertEntry.url} target="_blank" rel="noreferrer"
              style={{
                flex: 1, minWidth: 70, padding: "8px 6px", borderRadius: 7, background: C.surface2, color: C.accent,
                textDecoration: "none", textAlign: "center", fontWeight: 700, fontSize: 11, fontFamily: "var(--mono)",
          ...
          <button onClick={onClose}
            style={{
              padding: "8px 14px", borderRadius: 7, background: "none",
...
function DiscoverTab({ C, push }) {
  ...
  const doQuickScan = async () => {
    ...
    try {
      const [sugRes, bulkRes] = await Promise.all([
        req(`/suggest-limit/${clean}?target_roi=${targetRoi}`).catch(() => null),
        req("/discover/bulk", { method: "POST", body: JSON.stringify({ isbns: [clean] }) }).catch(() => null),
      ]);
      setResult({ suggestion: sugRes, bulk: bulkRes?.results?.[0] || null, isbn: clean });
    } catch (e) { setError(e.message); }
    setLoading(false);
  };

  // Bulk discover
  const doBulkDiscover = async () => {
    const isbns = isbnsText.split(/[\n,;\s]+/).map(s => s.replace(/[^0-9Xx]/g, "")).filter(s => s.length >= 10);
    if (!isbns.length) return;
    setLoading(true); setError(null); setBulkResults(null);
    try {
      const res = await req("/discover/bulk", { method: "POST", body: JSON.stringify({ isbns }) }, 120000);
      setBulkResults(res);
    } catch (e) { setError(e.message); }
    setLoading(false);
  };
  ...
  const doReverse = async () => {
    ...
    setLoading(true); setError(null); setBulkResults(null);
    try {
      const res = await req("/discover/reverse", { method: "POST", body: JSON.stringify({ isbns, target_roi: targetRoi }) }, 120000);
      setBulkResults(res);
    } catch (e) { setError(e.message); }
    setLoading(false);
  };

  const tierColor = (tier) => ({ fire: "#ef4444", good: "#22c55e", low: "#f59e0b", loss: "#6b7280" }[tier] || C.muted);
  const scoreColor = (score) => score >= 80 ? "#ef4444" : score >= 60 ? "#22c55e" : score >= 40 ? "#f59e0b" : C.muted;
    ...
    <div>
      ...
      {mode === "quick" && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 12 }}>⚡ ISBN Quick Scan</div>
          <div style={{ fontSize: 11, color: C.muted, marginBottom: 16 }}>ISBN gir → eBay fiyat + Amazon fiyat + ROI + dinamik limit önerisi</div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <input className="inp" placeholder="ISBN (10 veya 13)" value={isbn} onChange={e => setIsbn(e.target.value)} onKeyDown={e => e.key === "Enter" && doQuickScan()} style={{ flex: 1 }} />
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ fontSize: 11, color: C.muted }}>ROI%</span>
              <input className="inp" type="number" value={targetRoi} onChange={e => setTargetRoi(Number(e.target.value))} style={{ width: 60 }} />
            </div>
            <button className="add-btn" onClick={doQuickScan} disabled={loading}>
              {loading ? "⏳" : "🔍 Tara"}
            </button>
          </div>

          {error && <div style={{ color: C.red, fontSize: 12, marginBottom: 8 }}>⚠️ {error}</div>}

          {result && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 12 }}>📚 ISBN: {result.isbn}</div>

              {/* Suggestion card */}
              {result.suggestion?.ok && (
                <div style={{ background: C.cardBg, border: `1px solid ${C.cardBorder}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.accent, marginBottom: 8 }}>🎯 Dinamik Limit Önerisi</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Amazon Satış</div>
                      <div style={{ fontSize: 18, fontWeight: 700, color: C.green }}>${result.suggestion.sell_price}</div>
                      <div style={{ fontSize: 9, color: C.muted2 }}>{result.suggestion.sell_source}</div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Max Alım ({targetRoi}% ROI)</div>
                      <div style={{ fontSize: 22, fontWeight: 700, color: C.accent }}>${result.suggestion.max_buy}</div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Tahmini Kâr</div>
                      <div style={{ fontSize: 18, fontWeight: 700, color: result.suggestion.expected_profit > 0 ? C.green : C.red }}>${result.suggestion.expected_profit}</div>
                    </div>
                  </div>
                  <div style={{ marginTop: 10, fontSize: 10, color: C.muted2, display: "flex", gap: 12, justifyContent: "center" }}>
                    <span>Referral: ${result.suggestion.referral_fee}</span>
                    <span>Closing: ${result.suggestion.closing_fee}</span>
                    <span>FBA: ${result.suggestion.fulfillment}</span>
                    <span>Inbound: ${result.suggestion.inbound}</span>
                  </div>
                </div>
              )}
              ...
              {result.bulk?.ebay && (
                <div style={{ background: C.cardBg, border: `1px solid ${C.cardBorder}`, borderRadius: 8, padding: 16, marginBottom: 12 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.blue, marginBottom: 8 }}>📦 eBay Listings ({result.bulk.ebay.total_found} bulundu)</div>
                  {(result.bulk.ebay.cheapest || []).map((c, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: i < (result.bulk.ebay.cheapest.length - 1) ? `1px solid ${C.border}` : "none" }}>
                      <div>
                        <span style={{ fontSize: 16, fontWeight: 700, color: C.text }}>${c.total}</span>
                        <span className="badge" style={{ background: C.surface2, color: C.muted, marginLeft: 8 }}>{c.condition}</span>
                        {c.make_offer && <span className="badge" style={{ background: "#fbbf2422", color: "#fbbf24", marginLeft: 4 }}>OFFER</span>}
                        {c.ship_estimated && <span className="badge" style={{ background: C.surface2, color: C.orange, marginLeft: 4 }}>est.ship</span>}
                      </div>
                      {c.url && <a href={c.url} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: C.accent }}>eBay ↗</a>}
                    </div>
                  ))}
                </div>
              )}
              ...
              {/* Profit card */}
              {result.bulk?.best_deal && (
                <div style={{ background: C.cardBg, border: `1px solid ${tierColor(result.bulk.best_deal.roi_tier)}`, borderRadius: 8, padding: 16 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: tierColor(result.bulk.best_deal.roi_tier), marginBottom: 8 }}>
                    {result.bulk.best_deal.roi_tier === "fire" ? "🔥" : result.bulk.best_deal.roi_tier === "good" ? "✨" : result.bulk.best_deal.roi_tier === "low" ? "⚠️" : "❌"} Profit Analizi (En Ucuz eBay Listing)
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: C.muted }}>eBay Fiyat</div>
                      <div style={{ fontSize: 16, fontWeight: 600, color: C.text }}>${result.bulk.best_deal.ebay_cost}</div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: C.muted }}>Kâr</div>
                      <div style={{ fontSize: 16, fontWeight: 600, color: result.bulk.best_deal.profit > 0 ? C.green : C.red }}>${result.bulk.best_deal.profit}</div>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 10, color: C.muted }}>ROI</div>
                      <div style={{ fontSize: 16, fontWeight: 600, color: tierColor(result.bulk.best_deal.roi_tier) }}>{result.bulk.best_deal.roi_pct}%</div>
                    </div>
                  </div>
                </div>
              )}
      ...
      {mode === "bulk" && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 12 }}>📦 Toplu ISBN Keşif</div>
          <div style={{ fontSize: 11, color: C.muted, marginBottom: 16 }}>ISBN'leri yapıştır (satır başı, virgül veya boşlukla ayırarak) — max 200</div>
          <textarea className="inp" rows={6} placeholder={"9780132350884\n9780201633610\n9780596007126"} value={isbnsText} onChange={e => setIsbnsText(e.target.value)} style={{ width: "100%", resize: "vertical", marginBottom: 12 }} />
          <div style={{ display: "flex", gap: 8 }}>
            <button className="add-btn" onClick={doBulkDiscover} disabled={loading}>
              {loading ? "⏳ Taranıyor..." : `🔍 ${isbnsText.split(/[\n,;\s]+/).filter(s => s.replace(/[^0-9Xx]/g, "").length >= 10).length} ISBN Tara`}
            </button>
          </div>
      ...
      {mode === "reverse" && (
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 12 }}>🔄 Reverse Lookup</div>
          <div style={{ fontSize: 11, color: C.muted, marginBottom: 16 }}>ISBN'leri gir → eBay'de en ucuz listingleri bul → Amazon fiyatla karşılaştır → arbitraj fırsatlarını sırala</div>
          <textarea className="inp" rows={6} placeholder={"ISBN'leri satır başı ayırarak gir..."} value={isbnsText} onChange={e => setIsbnsText(e.target.value)} style={{ width: "100%", resize: "vertical", marginBottom: 12 }} />
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <span style={{ fontSize: 11, color: C.muted }}>Hedef ROI%</span>
              <input className="inp" type="number" value={targetRoi} onChange={e => setTargetRoi(Number(e.target.value))} style={{ width: 60 }} />
            </div>
            <button className="add-btn" onClick={doReverse} disabled={loading}>
              {loading ? "⏳ Taranıyor..." : "🔄 Reverse Tara"}
            </button>
          </div>
...
function _renderBulkResults(data, C) {
  ...
  return (
    <div style={{ marginTop: 16 }}>
      <div style={{ fontSize: 11, color: data.scanned === data.total ? "#22c55e" : "#f59e0b", marginBottom: 12 }}>
...
function _renderReverseResults(data, C) {
  ...
  return (
    <div style={{ marginTop: 16 }}>
      <div style={{ fontSize: 11, color: data.found > 0 ? "#22c55e" : "#f59e0b", marginBottom: 12 }}>
        ✅ {data.found}/{data.total_input} kaynakta bulundu · {data.duration_s}s
      </div>
      {results.length === 0 ? <div style={{ color: "#6b7280", fontSize: 12 }}>Sonuç yok</div> : (
        <div>
          {results.map((r, i) => {
...
function AppReal() {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.filter = blueFilter ? "sepia(0.4) hue-rotate(-15deg) saturate(1.3) contrast(0.95) brightness(0.9)" : "none";
  }, [blueFilter]);
      ...
      <div style={{ padding: "28px 32px", maxWidth: 1100, margin: "0 auto" }}>
        {loading ? <div style={{ color: C.muted3, textAlign: "center", paddingTop: 80, fontSize: 13 }}>Yükleniyor…</div> : (
          <>
            ...
            {tab === "discover" && <DiscoverTab C={C} push={push} />}
          </>
        )}
      </div>
    </div>
